...
# 🛠️ CREAR MEMORIA GLOBAL COMPARTIDA (Para todos los celulares por igual)
@st.cache_resource
def obtener_memoria_global():
    return {}

cronometros_globales = obtener_memoria_global()
...
try:
    ...
    for idx, fila in df_datos.iterrows():
        ...
        # 3. Motor de Memoria Global Sincronizado
        if "LISTO" in estado or "FINALIZADO" in estado:
            tiempo_mostrar = "✅ ¡Listo para recoger!"
            if identificador_pieza in cronometros_globales:
                del cronometros_globales[identificador_pieza]
        elif "IMPRIMIENDO" in estado or "IMPR..." in estado:
            
            # Si la pieza no está en la memoria GLOBAL, o si cambiaste las horas/minutos en el Excel:
            if (identificador_pieza not in cronometros_globales) or (cronometros_globales[identificador_pieza]["tiempo_inicial_excel"] != segundos_totales_excel):
                cronometros_globales[identificador_pieza] = {
                    "tiempo_inicial_excel": segundos_totales_excel,
                    "hora_registro_servidor": tiempo_actual_sistema  # Punto cero fijado para TODO el mundo
                }
            
            # Restamos el tiempo transcurrido desde que se guardó a nivel global
            tiempo_transcurrido = tiempo_actual_sistema - cronometros_globales[identificador_pieza]["hora_registro_servidor"]
