...
try:
    ...
    for idx, fila in df_datos.iterrows():
        ...
        if "LISTO" in estado or "FINALIZADO" in estado:
            ...
            # 🎯 DETECCIÓN DE CAMBIO CRÍTICA:
            # Si el usuario no existía en memoria, creamos su registro.
            # O SI EL TIEMPO DEL EXCEL CAMBIÓ respecto a lo guardado, FORZAMOS el reseteo del tiempo cero.
            if (identificador_pieza not in st.session_state.cronometros) or (st.session_state.cronometros[identificador_pieza]["tiempo_inicial_excel"] != segundos_totales_excel):
                st.session_state.cronometros[identificador_pieza] = {
                    ...
                    "hora_registro_servidor": tiempo_actual_sistema  # Resetea el "punto cero" al segundo actual
                }
            
            # Calcular cuánto tiempo real ha transcurrido estrictamente DESDE EL RESETEO
